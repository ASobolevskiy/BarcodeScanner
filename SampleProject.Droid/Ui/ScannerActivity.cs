...
using Java.Lang;
using SampleProject.Droid.Helpers;
...
public class ScannerActivity : AppCompatActivity
{
    ...
    private ScannerOverlayView? _barcodeOverlay;
    ...
    protected override void OnCreate(Bundle? savedInstanceState)
    {
        ...
        _barcodeOverlay = FindViewById<ScannerOverlayView>(ResourceConstant.Id.barcodeOverlay);
    ...
    private void OnBarcodeVisualUpdate(Barcode? barcode)
    {
        ...
        var matrix = MatrixHelper.GetCorrectionMatrix(_latestImageProxy, _cameraPreview);
        if (matrix == null)
            return;
        ...
        lock (_lastBoxParams)
        {
            if (!_lastBoxParams.TryGetValue(key, out var prev))
            {
                ...
                RunOnUiThread(() => { _barcodeOverlay.UpdateBarcode(barcode.RawValue, rectPoints); });
            }
            else
            {
